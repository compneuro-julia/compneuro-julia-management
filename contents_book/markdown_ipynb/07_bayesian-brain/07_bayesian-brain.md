...
#### エネルギーベースモデル
次に指数分布族と関連して扱いやすい枠組みであるエネルギーベースモデル (energy-based model; EBM) を紹介する．エネルギーベースモデルでは，確率変数 $\mathbf{x}$ の確率密度関数を，パラメータ $\theta$ を持つエネルギー関数 $E(\mathbf{x}; \theta):\ \mathbb{R}^{d}\mathbb{\rightarrow R}$ および Gibbs-Boltzmann分布（Softmax関数の元となった分布でもある）を用いて次のように表す \footnote{特に機械学習の文脈では，パラメータ $\theta$ によって分布が定まることを強調するため，あるいは記法を簡略化するために，しばしば $p_\theta(\cdot) := p(\cdot \mid \theta)$ と略記される．しかし，本書では $\theta$ を確率変数として扱う立場を明確にし，導出上の混乱を避けるため，この略記は用いない．}．
...
ただし，$Z(\theta)$は規格化定数あるいは分配関数 (partition function) である．定数とついているが，$\theta$ の関数であることに注意が必要である．

エネルギーベースモデルの利点は，確率変数およびパラメータを入力として1次元の値を返すエネルギー関数さえ設計すれば複雑な確率分布を構成する必要はないという点である．このエネルギー関数は，ある状態 $\mathbf{x}$ の「好ましさ」や「自然さ」を定量的に評価するものであり，エネルギーが小さいほどその状態がより実現しやすいと解釈される．そのように設計すれば確率分布として構成をすることができる．もちろん，なぜこのようなことができるのかと言えば，分配関数$Z(\theta)$によって確率密度関数としての整合性を確保しているためである．エネルギーの例としては，第2章で紹介したHopfieldモデルにおけるエネルギーなどがある．
...
とすればよい．要するに，指数の外側に $\mathbf{x}$ に依存する項を含まないようにしているのが特徴である．こうすることによる利点があり，それを享受しているのがスコアと呼ばれる概念である．エネルギーベースモデルにおいて，対数尤度の勾配 $\nabla_\mathbf{x} \ln p_\theta(\mathbf{x} \mid \theta)$ をスコアとよぶ．

$$
\begin{equation}
\nabla_\mathbf{x} \ln p(\mathbf{x} \mid \theta) =
- \nabla_\mathbf{x}  \ln E(\mathbf{x}; \theta) - \nabla_\mathbf{x} \ln Z(\theta)=- \nabla_\mathbf{x}  \ln E(\mathbf{x}; \theta)
\end{equation}
$$
...
理想的には，外界のすべての変数が観測可能（fully visible）であることが望ましいが，実世界の多くの場合においては観測できない変数が存在する．そのような隠れた変数を表現するために，潜在変数（latent variable） $\mathbf{z}$ を導入する．潜在変数に対して，$\mathbf{x}$ は観測変数 (observed variable) と呼ぶ．視覚系に対応させると，$\mathbf{x}$ は網膜像のような感覚入力を表し，$\mathbf{z}$ は物体のカテゴリ，三次元形状，照明条件など，より抽象的で高次の視覚的表現に相当すると解釈できる．ただし，この潜在変数は，実際の変数ではなく，推定する（神経モデルにおいては脳内に表現される）ものであることに注意をしてほしい．
...
ここで，この潜在変数モデルを訓練するにはどうすればよいのか，という問題には立ち入らず，先にこのモデルを階層的にしたモデルについて紹介する．
...
という境界条件を導入する。ここでの $\varnothing$ は「変数が存在しないこと」を表す形式的な記号であり，集合論的な空集合とは区別される。ただし，$\mathbf{z}_{L+1}$ として外部から入力を行う場合もあるので，常に存在しないとは限らない．なお，階層的潜在変数モデルは必ずしも直上の層のみに依存する必要はなく，スキップ結合を導入して「上位すべての層に依存する」と定式化することも可能である。
...
これは予測符号化 (predictive coding) と呼ばれる．

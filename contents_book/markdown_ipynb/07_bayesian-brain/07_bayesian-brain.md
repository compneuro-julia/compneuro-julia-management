...
#### 指数型分布族
多変量正規分布が属する確率分布の枠組みを指数型分布族 (exponential family) と呼ぶ．指数型分布族には多数の分布が属しており，他にはポアソン分布，指数分布，ガンマ分布，ベルヌーイ分布などが属する．指数型分布族の一般形は

確率変数とパラメータの両方がベクトルの場合は，

$$
p(\mathbf{x}\mid \theta) = h(x)\cdot g(\theta) \exp(\eta(\theta)^\top t(\mathbf{x}))
$$ 

$t()$ は $\theta$ の十分統計量 (sufficient statistic) である．
...
そのため，潜在変数モデルの推論には近似ベイズ推論 が必要となる。代表的な方法として，変分推論やマルコフ連鎖モンテカルロ法（MCMC）が挙げられる。

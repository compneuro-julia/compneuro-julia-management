...
\hline
スケール &  \begin{tabular}{c}中心的な\\疑問対象 \end{tabular} & \citep{Marr1982-wk}  & Pylyshyn & Anderson & Dayan \& Abbott \\ \hline
マクロ & \begin{tabular}{c}なぜ\\(Why) \end{tabular} & 計算理論 & 意味論的 & 目的論的  & 解釈的 \\ \cline{2-6}
\multirow{2}{*}{\scalebox{2}[5]{\blacktriangledown}} & どのように& \multirow{2}{*}{表現とアルゴリズム} & アルゴリズム & アルゴリズム & \multirow{2}{*}{記述的} \\ \cline{4-5}
& (How) & & 機能的構造  & 実装 &\\ \cline{2-6}
ミクロ & \begin{tabular}{c}何が\\(What) \end{tabular}  & ハードウェア実装 & 生物学的 & 生物学的 & 機構的\\ \hline
\end{tabular}
\end{table}

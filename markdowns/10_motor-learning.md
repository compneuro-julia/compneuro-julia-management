- [第10章：運動学習と最適制御](#第10章運動学習と最適制御)
  - [躍度最小モデル](#躍度最小モデル)
    - [等式制約下の二次計画法 (Equality Constrained Quadratic Programming)](#等式制約下の二次計画法-equality-constrained-quadratic-programming)
    - [躍度最小モデルの実装](#躍度最小モデルの実装)
  - [終点誤差分散最小モデル](#終点誤差分散最小モデル)
  - [最適フィードバック制御モデル](#最適フィードバック制御モデル)
    - [最適フィードバック制御モデルの構造](#最適フィードバック制御モデルの構造)
    - [系の状態変化](#系の状態変化)
    - [LQG](#lqg)
      - [運動制御 (Linear-Quadratic Regulator)](#運動制御-linear-quadratic-regulator)
      - [状態推定 (Kalman Filter)](#状態推定-kalman-filter)
      - [一般化LQG](#一般化lqg)
      - [シミュレーション](#シミュレーション)
  - [無限時間最適フィードバック制御モデル](#無限時間最適フィードバック制御モデル)
    - [モデルの構造](#モデルの構造)
      - [K, L, S, Pの計算](#k-l-s-pの計算)
    - [Target jump](#target-jump)
  - [げっ歯類の自由行動の現象論的モデル](#げっ歯類の自由行動の現象論的モデル)
...
## げっ歯類の自由行動の現象論的モデル
本節は運動学習や最適制御から離れた話題であるが，「運動」という枠組みに入れることができるために紹介を行う．ラットが自由行動下において箱の中を探索する際の軌跡をシミュレーションする {cite:p}`Raudies2012-gp`．これまでと異なり，現象論的に運動を生成する．場所細胞・格子細胞等自己位置と神経活動が相関する細胞のシミュレーションにおいて用いられる {cite:p}`George2024-rv`．

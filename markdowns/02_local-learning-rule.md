...
https://www.jneurosci.org/content/17/11/4382.full
https://www.sciencedirect.com/science/article/pii/S0896627309002876#bib24
...
#### 抑制安定化ネットワーク
Wilson–Cowanモデルの解析において、ある種のパラメータ条件下で現れる特異な安定性の機構が、抑制安定化ネットワーク (inhibition-stabilized networ; ISN) と呼ばれる動作様式である。

ISNとは、抑制性神経集団がネットワーク全体の安定性を担うような神経回路構成を指す。言い換えれば、興奮性ニューロン同士の結合が強いために本来は活動が発散してしまうところを、抑制性ニューロンの負帰還によってネットワークが安定化されている状態を意味する。Wilson–Cowanモデルにおいては、再帰的な興奮性結合 $w_{\mathrm{EE}}$ が大きく、単独では不安定となるような場合であっても、十分に強い抑制性の結合 $w_{\mathrm{EI}}, w_{\mathrm{IE}}, w_{\mathrm{II}}$ が存在すれば、全体の平衡点は安定に保たれる。このとき、ネットワークはISNとして機能していると解釈される。

数学的には、ISNかどうかはヤコビ行列の固有値の符号解析や興奮性結合の強度を変化させたときの抑制性ニューロンの応答符号によって判定できる。典型的なISNでは、外部から興奮性集団に入力を加えると、抑制性集団の活動も一見逆説的に増加する（これを“paradoxical response”と呼ぶ）。これは、興奮性入力によってネットワーク全体の活動が高まり、それを打ち消すために抑制性ニューロンがより強く活動するというダイナミクスを反映している。

このようなISNの機構は、皮質における実際の神経回路の働きを説明するうえで重要な枠組みを提供する。たとえば、実験的にも一次視覚野や前頭前野においてISN的な応答が報告されており、抑制性ニューロンが単なる抑制要素ではなく、ネットワークの安定性を制御する動的な役割を果たしていることが示唆されている。Wilson–Cowanモデルはその構造上、ISN的な状態を自然に含みうるため、ISNの理論的解析においてもしばしば基礎モデルとして用いられる。
...
**Neural Mass Model**は，ミニコラムや皮質局所回路といった小規模な神経集団を1ユニットとしてモデル化し，**集団平均の膜電位や発火率の時間変化**を常微分方程式（ODE）で記述する．代表的な構造はWilson–Cowanモデルに基づいて

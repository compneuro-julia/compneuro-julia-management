- [第6章：スパイキングニューロンとシナプスダイナミクス](#第6章スパイキングニューロンとシナプスダイナミクス)
  - [コンダクタンスベースモデル※](#コンダクタンスベースモデル)
    ...
    - [Connor-Stevensモデル](#connor-stevensモデル)
    - [F-I曲線](#f-i曲線)
    - [全か無かの法則の反例](#全か無かの法則の反例)
    - [FitzHugh-Nagumoモデル](#fitzhugh-nagumoモデル)
  - [積分発火モデル](#積分発火モデル)
    - [Leaky integrate-and-fire モデル](#leaky-integrate-and-fire-モデル)
    - [解析的計算によるF-I curveの描画](#解析的計算によるf-i-curveの描画)
  - [Izhikevich モデル※](#izhikevich-モデル)
    - [Izhikevich モデルの定義](#izhikevich-モデルの定義)
  ...
  - [ポアソン過程モデル](#ポアソン過程モデル)
    - [点過程とポアソン過程](#点過程とポアソン過程)
    - [定常ポアソン過程](#定常ポアソン過程)
      - [ISIの累積により発火時刻を求める手法](#isiの累積により発火時刻を求める手法)
      - [$\\Delta t$ 間の発火確率が $\\lambda\\Delta t$ であることを利用する方法](#delta-t-間の発火確率が-lambdadelta-t-であることを利用する方法)
    - [非定常ポアソン過程](#非定常ポアソン過程)
    - [死時間付きポアソン過程モデル (Poisson process with dead time, PPD)](#死時間付きポアソン過程モデル-poisson-process-with-dead-time-ppd)
    - [ガンマ過程モデル](#ガンマ過程モデル)
  - [シナプスの生理とCurrent/Conductance-based シナプス](#シナプスの生理とcurrentconductance-based-シナプス)
    - [シナプスの形態と生理](#シナプスの形態と生理)
- [Current / Conductance-based シナプス](#current--conductance-based-シナプス)
  - [化学シナプスの2つの記述形式](#化学シナプスの2つの記述形式)
  - [Current-based シナプス](#current-based-シナプス)
  - [Conductance-based シナプス](#conductance-based-シナプス)
  - [指数関数型シナプスモデル](#指数関数型シナプスモデル)
  - [単一指数関数型モデル(Single exponential model)](#単一指数関数型モデルsingle-exponential-model)
    - [微分方程式による表現](#微分方程式による表現)
  - [二重指数関数型モデル(Double exponential model)](#二重指数関数型モデルdouble-exponential-model)
    - [$\\alpha$関数](#alpha関数)
    - [微分方程式による表現](#微分方程式による表現-1)
  - [動力学シナプスモデル](#動力学シナプスモデル)
  - [チャネル動態の動力学的表現](#チャネル動態の動力学的表現)
    - [Hodgkin-Huxleyモデルにおけるシナプスモデル](#hodgkin-huxleyモデルにおけるシナプスモデル)
  - [短期的シナプス可塑性](#短期的シナプス可塑性)
  - [シナプス入力の重みづけ](#シナプス入力の重みづけ)
...
1. $[t, t+\Delta t)$に点が1つ生じる確率 : $P[N(t+\Delta t)-N(t)=1]=\lambda(t)\Delta t+o(\Delta t)$
1. 微小時間$\Delta t$の間に点は2つ以上生じない : $P[N(t+\Delta t)-N(t)=2]=o(\Delta t)$
1. 任意の時点$t_1 < t_2 < \cdots< t_n$に対して，増分 $N(t_2)-N(t_1), N(t_3)-N(t_2), \cdots, N(t_n)-N(t_{n−1})$は互いに独立である．

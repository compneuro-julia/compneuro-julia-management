- [第4章：ニューラルネットワークと貢献度分配問題](#第4章ニューラルネットワークと貢献度分配問題)
  - [貢献度分配問題と誤差逆伝播法](#貢献度分配問題と誤差逆伝播法)
    - [勾配法と誤差逆伝播法](#勾配法と誤差逆伝播法)
    - [順伝播 (forward propagation)](#順伝播-forward-propagation)
    - [逆伝播 (backward propagation)](#逆伝播-backward-propagation)
    - [損失関数](#損失関数)
    - [オプティマイザ](#オプティマイザ)
    - [例1. スパイラルデータセット (分類課題)](#例1-スパイラルデータセット-分類課題)
    - [例2. Zipser-Andersenモデル](#例2-zipser-andersenモデル)
      - [データセットの生成](#データセットの生成)
    - [例3. MNIST](#例3-mnist)
    - [例4. 自己符号化器](#例4-自己符号化器)
    - [線形多層ニューラルネットワークの学習ダイナミクス](#線形多層ニューラルネットワークの学習ダイナミクス)
  - [線形多層ニューラルネットワークにおける学習ダイナミクスと知識の獲得](#線形多層ニューラルネットワークにおける学習ダイナミクスと知識の獲得)
    - [モデルと学習](#モデルと学習)
    - [特異値分解(SVD)による学習ダイナミクスの解析](#特異値分解svdによる学習ダイナミクスの解析)
  - [線形多層ニューラルネットワークにおける勾配降下法による低ランク解の獲得](#線形多層ニューラルネットワークにおける勾配降下法による低ランク解の獲得)
  - [非対称な逆向き投射による誤差伝播](#非対称な逆向き投射による誤差伝播)
  - [予測符号化による活動と結合の共調整](#予測符号化による活動と結合の共調整)
    - [予測符号化による訓練](#予測符号化による訓練)
    - [順伝播 (forward propagation)](#順伝播-forward-propagation-1)
      - [予測符号化による訓練](#予測符号化による訓練-1)
  - [摂動を用いた学習則](#摂動を用いた学習則)
    - [ノード摂動法と重み摂動法](#ノード摂動法と重み摂動法)
      - [不偏推定量であることの証明](#不偏推定量であることの証明)
---

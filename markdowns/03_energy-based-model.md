- [第3章：エネルギーベースモデル](#第3章エネルギーベースモデル)
  - [Hopfield モデル](#hopfield-モデル)
    - [稠密連想記憶 (dense associative memory) モデル](#稠密連想記憶-dense-associative-memory-モデル)
  - [Boltzmann マシン](#boltzmann-マシン)
    - [制限 Boltzmann マシン](#制限-boltzmann-マシン)
    - [訓練データで学習](#訓練データで学習)
  - [スパース符号化モデル](#スパース符号化モデル)
    - [Sparse codingと生成モデル](#sparse-codingと生成モデル)
    - [確率的モデルの記述](#確率的モデルの記述)
    - [事前分布の設定](#事前分布の設定)
    - [目的関数の設定と最適化](#目的関数の設定と最適化)
    - [Locally competitive algorithm (LCA)](#locally-competitive-algorithm-lca)
      - [軟判定閾値関数を用いる場合 (ISTA)](#軟判定閾値関数を用いる場合-ista)
    - [重み行列の更新則](#重み行列の更新則)
    - [Sparse coding networkの実装](#sparse-coding-networkの実装)
  - [予測符号化モデル](#予測符号化モデル)
    - [観測世界の階層的予測](#観測世界の階層的予測)
    - [損失関数と学習則](#損失関数と学習則)
      - [事前分布の設定](#事前分布の設定-1)
      - [損失関数の設定](#損失関数の設定)
      - [再帰ネットワークの更新則](#再帰ネットワークの更新則)

- [第9章：神経回路網によるベイズ推論](#第9章神経回路網によるベイズ推論)
  - [ベイズ脳仮説と不確実性の表現](#ベイズ脳仮説と不確実性の表現)
    - [ベイズ脳仮説](#ベイズ脳仮説)
    - [神経活動による不確実性の表現](#神経活動による不確実性の表現)
  - [ベイズ脳仮説と神経活動による不確実性の表現](#ベイズ脳仮説と神経活動による不確実性の表現)
  - [神経回路における不確実性の表現](#神経回路における不確実性の表現)
  - [エネルギーベースモデル](#エネルギーベースモデル)
  - [エネルギーベースモデルとサンプリング](#エネルギーベースモデルとサンプリング)
  - [ベイズ線形回帰](#ベイズ線形回帰)
  - [マルコフ連鎖モンテカルロ法](#マルコフ連鎖モンテカルロ法)
    - [マルコフ連鎖モンテカルロ法 (MCMC)](#マルコフ連鎖モンテカルロ法-mcmc)
    - [モンテカルロ法](#モンテカルロ法)
    - [マルコフ連鎖](#マルコフ連鎖)
    - [Metropolis-Hastings法](#metropolis-hastings法)
    - [ランジュバン・モンテカルロ法 (LMC)](#ランジュバンモンテカルロ法-lmc)
    - [ハミルトニアン・モンテカルロ法 (HMC法)](#ハミルトニアンモンテカルロ法-hmc法)
  - [神経サンプリング](#神経サンプリング)
  - [ガウス尺度混合モデル](#ガウス尺度混合モデル)
    - [事前分布](#事前分布)
    - [分散共分散行列$\\mathbf{C}$の作成](#分散共分散行列mathbfcの作成)
    - [事後分布の計算](#事後分布の計算)
  - [興奮性・抑制性神経回路によるサンプリング](#興奮性抑制性神経回路によるサンプリング)
  - [Spikingニューラルネットワークにおけるサンプリング](#spikingニューラルネットワークにおけるサンプリング)
  - [シナプスサンプリング](#シナプスサンプリング)
  - [確率的集団符号化](#確率的集団符号化)
    - [確率的集団符号化 (probabilistic population coding)](#確率的集団符号化-probabilistic-population-coding)

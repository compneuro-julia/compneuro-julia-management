...
状態 $s_t$ において行動 $a_t$ を行い，時刻 $t$ に受け取る報酬を $r_t$ とする \footnote{状態 $s_t$ において行動 $a_t$ を行った後に受け取る報酬を$r_{t+1}$ とする流派もある．}．
状態 $s_t$ と行動 $a_t$ が報酬が

この$r_t$

環境の状態遷移確率は $p(s_{t+1}, r_t \mid s_t, a_t)$ で
...
$$
V^\pi(s) := \mathbb{E}_\pi \left[G_t \mid s_t = s \right]=\mathbb{E}_\pi \left[\sum_{k=t}^{\infty}\gamma^{k-t} r_{k} \mid s_t = s \right]
$$

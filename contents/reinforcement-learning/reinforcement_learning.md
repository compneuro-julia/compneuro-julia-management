...
#### 環境とエージェント
強化学習では環境 (environment) と，その中で行動するエージェント (agent) が定義される．環境とは，エージェントが相互作用する対象であり，エージェントの行動によってその状態が変化するものである．一方，エージェントは環境内で行動を選択し，学習を行う主体（例えば生物やロボットなど）を意味する．環境とエージェントの状態 (state) を $s\in \mathcal{S}$とし，エージェントの行動 (action) を $a \in \mathcal{A}$ とする．ここで、$\mathcal{S}$は環境のあらゆる可能な状態の集合であり，$\mathcal{A}$ はエージェントが選択できる行動の集合である．状態や行動は離散的または連続的であり得る．

例えば，グリッド状の迷路を探索する場合，環境は迷路全体を指し，状態集合 $\mathcal{S}$ は迷路内の各セルおよびエージェントの位置からなる．行動集合 $\mathcal{A}$ は、$\{上, 下, 左, 右\}$ の4つの移動方向からなる．この場合は状態も行動も離散的である．

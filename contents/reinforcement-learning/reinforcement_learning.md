...
状態 $s_t$ において行動 $a_t$ を行い，結果として報酬 $r_{t+1}$ を受け取り，状態 $s_{t+1}$ に遷移する \footnote{状態 $s_t$ において行動 $a_t$ を行った後に受け取る報酬を$r_t$ とする流派もある．}．
状態 $s_t$ と行動 $a_t$ が報酬が

$s_{t+1}$ と $r_{t+1}$ は直前の状態 $s_t$ と 行動 $a_t$ にのみ依存し，過去の


この$r_t$

環境の状態遷移確率は $p(s_{t+1}, r_{t+1} \mid s_t, a_t)$ で

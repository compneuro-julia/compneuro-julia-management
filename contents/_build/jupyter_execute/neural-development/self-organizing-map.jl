...
function plot_som(v, w)
    scatter(v[:, 1], v[:, 2], s=5)
    scatter(w[:, :, 1], w[:, :, 2], s=5) # w[i, j, 1]とw[i, j, 2]の点をプロット
...
function SOM!(v, w; η0=1.0, σ0=6, T=500)
    ...
    @showprogress for t in 1:T
        ...
        for i in 1:N
            ...
            α = η * mask[1:length(idx[1]), 1:length(idx[2])]
            for j in 1:dims
                w[idx..., j] += α .* (v[i, j] .- w[idx..., j])
            end
        end
        append!(w_history, [copy(w)])
    end
    return w_history
end;

w_history = SOM!(v, w, η0=1.0, σ0=6, T=500);

...
subplot(3,1,3); plot(t, I[:, 1]); ylabel("Injection\n current (pA)"); xlabel("Times (ms)")
...
for t=1:1000 # simulation of 1000 ms
    I=[5randn(Ne,1); 2randn(Ni,1)] # thalamic input
    fired = findall(v[:, 1] .>= 30) # indices of spikes
    firings = t==1 ? [t .+ 0*fired fired] : [firings; [t .+ 0*fired fired]]
    v[fired] = c[fired]
    u[fired] += d[fired]
    I = I + sum(S[:,fired], dims=2)
    v += 0.5(0.04v.^2 + 5v .+140 - u + I) # step 0.5 ms for numerical stability
    v += 0.5(0.04v.^2 + 5v .+140 - u + I) 
    u += a .* (b .* v - u)
end

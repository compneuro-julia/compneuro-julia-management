...
@kwdef mutable struct LIF{FT}
    ...
    tcount::Vector{FT} = zeros(1) # 時間カウント
end
...
次に変数を更新する関数`updateLIF!`を書く。

function updateLIF!(variable::LIF, param::LIFParameter, I::Vector, dt)
...
I = repeat(25f0 * ((t .> 50) - (t .> 200)) + 0f0 * ((t .> 250) - (t .> 400)), 1, N)  # injection current
...
# simulation
@time for i = 1:nt
    updateLIF!(neurons, neurons.param, I[i, :], dt)
    varr[i, :] = neurons.v_
end

...
シナプスのモデルは複数あるが, 良く用いられるのが\textbf{\index{しすうかんすうかたしなぷすもでる@指数関数型シナプスモデル}}(exponential synapse model)である．このモデルは生理学的な過程を無視した現象論的モデルであることに注意しよう．指数関数型シナプスモデルには2つの種類, \textbf{\index{たんいつしすうかんすうかたもでる@単一指数関数型モデル}} (single exponential model)と\textbf{\index{にじゅうしすうかんすうかたもでる@二重指数関数型モデル}} (double exponential model)がある．

数式の説明の前にモデルの挙動を示す．次図は2種類のモデルにおいて$t=0$でスパイクが生じてからのシナプス後電流の変化を示している．ただし, 実際のシナプス後電流はこれに\textbf{\index{しなぷすきょうど@シナプス強度}} (Synaptic strength)[^synstr]を乗じて総和を取ったものとなる．
...
上記の式において, $\tau=\tau_{r}=\tau_{d}$の場合は $\boldsymbol{\alpha}$ \textbf{\index{かんすう@関数}} (alpha function, alpha synapse)と呼ぶ ([Rall, 1967](https://pubmed.ncbi.nlm.nih.gov/6055351/))．式としては次のようになる．

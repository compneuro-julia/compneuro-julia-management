...
TD (Temporal difference) learningにおいて，\textbf{\index{ほうしゅうよそくごさ@報酬予測誤差}}(reward prediction error, \textbf{\index{RPE}}) $\delta_{i}$は次のように計算される． 
...
また，$V_{j}\left(x^{\prime}\right)$は予測価値分布$V\left(x^{\prime}\right)$からのサンプルです． このRPEは脳内において主に中脳の\textbf{\index{VTA}}(腹側被蓋野)や\textbf{\index{SNc}}(黒質緻密部)における\textbf{\index{どぱみん(dopamine)にゅーろん@ドパミン(dopamine)ニューロン}}の発火率として表現されています．
